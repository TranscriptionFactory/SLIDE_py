...
```bash
$HOME/.local/bin/claude-flow hooks pre-task --description "[task description]"
```
...
$HOME/.local/bin/claude-flow swarm init --topology hierarchical --max-agents 8 --strategy specialized

# Large teams (10-15 agents) - use hierarchical-mesh for V3 queen + peer communication
$HOME/.local/bin/claude-flow swarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
```
...
Bash("$HOME/.local/bin/claude-flow swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
Bash("$HOME/.local/bin/claude-flow memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("$HOME/.local/bin/claude-flow memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("$HOME/.local/bin/claude-flow hooks route --task '[task description]'")
```

### After Completing Any Task Successfully
```bash
# 1. Store successful pattern for future reference
Bash("$HOME/.local/bin/claude-flow memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("$HOME/.local/bin/claude-flow hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("$HOME/.local/bin/claude-flow hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("$HOME/.local/bin/claude-flow hooks worker dispatch --trigger optimize")
```
...
```bash
# Initialize project
$HOME/.local/bin/claude-flow init --wizard

# Start daemon with background workers
$HOME/.local/bin/claude-flow daemon start

# Spawn an agent
$HOME/.local/bin/claude-flow agent spawn -t coder --name my-coder

# Initialize swarm
$HOME/.local/bin/claude-flow swarm init --v3-mode

# Search memory (HNSW-indexed)
$HOME/.local/bin/claude-flow memory search --query "authentication patterns"

# System diagnostics
$HOME/.local/bin/claude-flow doctor --fix

# Security scan
$HOME/.local/bin/claude-flow security scan --depth full

# Performance benchmark
$HOME/.local/bin/claude-flow performance benchmark --suite all
```
...
```bash
# Core hooks
$HOME/.local/bin/claude-flow hooks pre-task --description "[task]"
$HOME/.local/bin/claude-flow hooks post-task --task-id "[id]" --success true
$HOME/.local/bin/claude-flow hooks post-edit --file "[file]" --train-neural true

# Session management
$HOME/.local/bin/claude-flow hooks session-start --session-id "[id]"
$HOME/.local/bin/claude-flow hooks session-end --export-metrics true
$HOME/.local/bin/claude-flow hooks session-restore --session-id "[id]"

# Intelligence routing
$HOME/.local/bin/claude-flow hooks route --task "[task]"
$HOME/.local/bin/claude-flow hooks explain --topic "[topic]"

# Neural learning
$HOME/.local/bin/claude-flow hooks pretrain --model-type moe --epochs 10
$HOME/.local/bin/claude-flow hooks build-agents --agent-types coder,tester

# Background workers
$HOME/.local/bin/claude-flow hooks worker list
$HOME/.local/bin/claude-flow hooks worker dispatch --trigger audit
$HOME/.local/bin/claude-flow hooks worker status

# Coverage-aware routing
$HOME/.local/bin/claude-flow hooks coverage-gaps --format table
$HOME/.local/bin/claude-flow hooks coverage-route --task "[task]"

# Statusline (for Claude Code integration)
$HOME/.local/bin/claude-flow hooks statusline
$HOME/.local/bin/claude-flow hooks statusline --json
```
...
$HOME/.local/bin/claude-flow migrate status

# Run migration with backup
$HOME/.local/bin/claude-flow migrate run --backup

# Rollback if needed
$HOME/.local/bin/claude-flow migrate rollback

# Validate migration
$HOME/.local/bin/claude-flow migrate validate
```
...
Bash("$HOME/.local/bin/claude-flow hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("$HOME/.local/bin/claude-flow performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("$HOME/.local/bin/claude-flow performance profile --target '[component]'")
```

### Session Persistence (Cross-Conversation Learning)
```bash
# At session start - restore previous context
Bash("$HOME/.local/bin/claude-flow session restore --latest")

# At session end - persist learned patterns
Bash("$HOME/.local/bin/claude-flow hooks session-end --generate-summary true --persist-state true --export-metrics true")
```

### Neural Pattern Training
```bash
# Train on successful code patterns
Bash("$HOME/.local/bin/claude-flow neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("$HOME/.local/bin/claude-flow neural predict --input '[task description]'")

# View learned patterns
Bash("$HOME/.local/bin/claude-flow neural patterns --list")
```
...
Run `$HOME/.local/bin/claude-flow doctor` to check:
- Node.js version (20+)
- npm version (9+)
- Git installation
...
claude mcp add claude-flow -- $HOME/.local/bin/claude-flow
...
- **Swarm init**: `$HOME/.local/bin/claude-flow swarm init --topology <type>`
- **Swarm status**: `$HOME/.local/bin/claude-flow swarm status`
- **Agent spawn**: `$HOME/.local/bin/claude-flow agent spawn -t <type> --name <name>`
- **Memory store**: `$HOME/.local/bin/claude-flow memory store --key "mykey" --value "myvalue" --namespace patterns`
- **Memory search**: `$HOME/.local/bin/claude-flow memory search --query "search terms"`
- **Memory list**: `$HOME/.local/bin/claude-flow memory list --namespace patterns`
- **Memory retrieve**: `$HOME/.local/bin/claude-flow memory retrieve --key "mykey" --namespace patterns`
- **Hooks**: `$HOME/.local/bin/claude-flow hooks <hook-name> [options]`
...
$HOME/.local/bin/claude-flow memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
$HOME/.local/bin/claude-flow memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
```
...
$HOME/.local/bin/claude-flow memory search --query "authentication patterns"
$HOME/.local/bin/claude-flow memory search --query "error handling" --namespace patterns --limit 5
```

### List Entries
```bash
# OPTIONAL: --namespace, --limit
$HOME/.local/bin/claude-flow memory list
$HOME/.local/bin/claude-flow memory list --namespace patterns --limit 10
```
...
$HOME/.local/bin/claude-flow memory retrieve --key "pattern-auth"
$HOME/.local/bin/claude-flow memory retrieve --key "pattern-auth" --namespace patterns
```

### Initialize Memory Database
```bash
$HOME/.local/bin/claude-flow memory init --force --verbose
```
