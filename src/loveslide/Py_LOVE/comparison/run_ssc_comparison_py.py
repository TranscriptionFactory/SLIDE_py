#!/usr/bin/env python3
"""
Python script to run LOVE on SSc data and compare with R outputs.

Run the R script first (run_ssc_comparison_R.R), then run this script.

Usage:
    python run_ssc_comparison_py.py [--generate-only]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Compare LOVE R and Python on SSc data")
    ...
    output_dir = script_dir / "outputs_ssc"

    print("=" * 60)
    print("LOVE Python - SSc Real Data Analysis")
    print("=" * 60)

    # Load SSc data
    data_file = script_dir / "SSc_X_SLIDE.csv"
    print(f"\nLoading data from: {data_file}")

    X_df = pd.read_csv(data_file, index_col=0)
    X = X_df.values

    print(f"Data dimensions: {X.shape[0]} samples x {X.shape[1]} features")
    print(f"Sample names: {', '.join(X_df.index[:5].tolist())}, ...")
    print(f"Feature names: {', '.join(X_df.columns[:5].tolist())}, ...")
    ...
    # Save outputs
    np.savetxt(output_dir / "ssc_hetero_A_py.csv", result_hetero['A'], delimiter=',')
    np.savetxt(output_dir / "ssc_hetero_C_py.csv", result_hetero['C'], delimiter=',')
    np.savetxt(output_dir / "ssc_hetero_Gamma_py.csv", result_hetero['Gamma'], delimiter=',')
    pd.DataFrame({'pureVec': result_hetero['pureVec']}).to_csv(
        output_dir / "ssc_hetero_pureVec_py.csv", index=False)
    pd.DataFrame({'K': [result_hetero['K']], 'optDelta': [result_hetero['optDelta']]}).to_csv(
        output_dir / "ssc_hetero_params_py.csv", index=False)
    ...
    # Save outputs
    np.savetxt(output_dir / "ssc_homo_A_py.csv", result_homo['A'], delimiter=',')
    np.savetxt(output_dir / "ssc_homo_C_py.csv", result_homo['C'], delimiter=',')
    np.savetxt(output_dir / "ssc_homo_Gamma_py.csv", result_homo['Gamma'], delimiter=',')
    pd.DataFrame({'pureVec': result_homo['pureVec']}).to_csv(
        output_dir / "ssc_homo_pureVec_py.csv", index=False)
    pd.DataFrame({'K': [result_homo['K']], 'optDelta': [result_homo['optDelta']]}).to_csv(
        output_dir / "ssc_homo_params_py.csv", index=False)

    if args.generate_only:
        print("\n" + "=" * 60)
        print("Python outputs saved to:", output_dir)
        print("=" * 60)
        return
    ...
    R_hetero_params = load_csv(output_dir / "ssc_hetero_params.csv")
    if R_hetero_params is None:
        print("\nERROR: R outputs not found. Run R script first:")
        print("  Rscript run_ssc_comparison_R.R")
        sys.exit(1)
    ...
    R_hetero_A = load_csv(output_dir / "ssc_hetero_A.csv")
    R_hetero_C = load_csv(output_dir / "ssc_hetero_C.csv")
    R_hetero_pureVec = load_csv(output_dir / "ssc_hetero_pureVec.csv")
    ...
    if R_hetero_A is not None:
        ...
        if R_hetero_A.shape == result_hetero['A'].shape:
            print(" - MATCH")
            # Compare actual values (with tolerance for numerical differences)
    ...
    R_homo_params = load_csv(output_dir / "ssc_homo_params.csv")
    R_homo_A = load_csv(output_dir / "ssc_homo_A.csv")
    R_homo_C = load_csv(output_dir / "ssc_homo_C.csv")
    R_homo_pureVec = load_csv(output_dir / "ssc_homo_pureVec.csv")

    if R_homo_params is not None:
        # Compare K
